...
4. **Sketch a plane.** One finger turns the object. The sketch plane is a line
   ...
   drag moves it and twist tilts it — **both at once**, each with its own dead
   zone, so a hand that wants to nudge and tilt gets both. The camera is not in
   it at all: see *two fingers never move the camera*, below. Zoom is the
   magnifier under the view cube. Neither gesture touches the body — the size is
   a number you drag and the turn is the body tool; a body that grows when you
   want a closer look is a body doing something you did not ask for. Three fingers move the
...
**Two fingers never move the camera.** Not at the end of a gesture, not for one
frame in the middle of it, and not even when the gesture is a pure pinch. Two
fingers belong to the object.

The pinch used to zoom the view, and it had to go. The three channels share one
motion, and they do not declare themselves at the same speed: the drag has to
travel six pixels, while the pinch has to change the gap between the fingers by
four per cent — and on a hand spanning 160 pixels that is six and a half pixels
*split between two fingers*, three each. Nobody puts two fingers on glass and
drags them without them sliding three pixels apart first. So the pinch won the
opening of every single drag: the camera dollied a notch, the drag then took the
gesture, and the camera was put back. **The putting back was the thing you could
see** — a jump out and a jump in, every time two fingers touched anything. The
tests never caught it, because they measured where the camera *ended*, and it
ended exactly where it started.

That can be softened — bigger dead zones, a window before the camera may have
the gesture — but it cannot be removed, because no dead zone can read what the
hand *meant*. So the camera is out of the contest entirely. Zoom is not gone; it
has its own control and no arbitration: the magnifier under the view cube, press
and drag like the bend, and the wheel on a bench. One way in per thing.

**The body tool is unchanged**, and that is not an exception to the rule: there
the pinch **is** the subject. The piece grows while you move and turn it,
because that is three things on the same piece, not two things fighting over
what you were pointing at.

The tests read the camera distance after every step while the fingers are still
down. Measured on the code before this: 0.689 units of swing mid-drag, with both
end-state tests reading 14.450 → 14.450. Now a drag, a pinch, and a pinch
followed by a drag all read 14.450 → 14.450, and the worst mid-gesture deviation
is 0.0000.
...
is down. When the column runs past the sheet it breaks into a second column to
the left instead of off the bottom, so a tool never has to share a button or
shrink to fit. It is the same distance-to-the-chord measure the contour was drawn
...
- **Four ways to draw: firkant, rund, halv, kontur.** `rund` is one drag like
